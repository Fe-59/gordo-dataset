...
DEFAULT_SECRETS_LOADER: ADLSecretsLoader = ADLEnvSecretsLoader()\
    .from_env("adl1", "dataplatformdlsprod", "DL_SERVICE_AUTH_STR")\
    .from_env("adl2", "omniadlseun", "DL2_SERVICE_AUTH_STR")


def create_storage(storage_type: Optional[str] = None, secrets_loader: Optional[ADLSecretsLoader] = None, **kwargs) -> FileSystem:
    """
    ...
    if not isinstance(secrets_loader, ADLSecretsLoader):
        raise ConfigException("secrets_loader should be instance of ADLSecretsLoader and not %s type", type(secrets_loader))
    storage: FileSystem
    if storage_type == "adl1":
        if "store_name" not in kwargs:
            kwargs["store_name"] = "dataplatformdlsprod"
        kwargs["adl_secret"] = secrets_loader.get_secret(storage_type, kwargs["store_name"])
        ...
        kwargs["adl_secret"] = secrets_loader.get_secret(storage_type, kwargs["account_name"])
